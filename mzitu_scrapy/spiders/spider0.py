...
class Spider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=('http://www.mzitu.com/\d{1,6}',), deny=('http://www.mzitu.com/\d{1,6}/\d{1,6}')), callback='parse_item', follow=False),
    )
    ...
    def parse_item(self, response):
        ...
        for num in range(1, int(max_num)):
            ...
            yield Request(page_url, callback=self.img_url,meta={"name":name,"url":url})
